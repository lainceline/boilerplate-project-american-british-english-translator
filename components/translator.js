...
function translate(text, locale) {
  ...
  if (locale === 'american-to-british') {
    ...
    for (const [am, br] of Object.entries(americanToBritishSpelling)) {
      ...
      if (regex.test(text)) {
        translation = translation.replace(regex, br);
        found = true;
      }
    }
    ...
    for (const [am, br] of Object.entries(americanToBritishTitles)) {
        ...
        if (regex.test(text)) {
            translation = translation.replace(regex, capitalize(br));
            found = true;
        }
    }
    // Handle time format
    ...
    if (timeRegex.test(text)) {
      translation = translation.replace(timeRegex, '$1.$2');
      found = true;
    }
    ...
    for (const [am, br] of Object.entries(americanToBritishSpelling)) {
      ...
      if (regex.test(text)) {
        translation = translation.replace(regex, am);
        found = true;
      }
    }
    ...
    for (const [am, br] of Object.entries(americanToBritishTitles)) {
      ...
      if (regex.test(text)) {
        ...
        translation = translation.replace(regex, capitalizedAm);
        found = true;
      }
    }
    ...
    if (britRegex.test(text)) {
      translation = translation.replace(britRegex, '$1:$2');
      found = true;
    }
  }
